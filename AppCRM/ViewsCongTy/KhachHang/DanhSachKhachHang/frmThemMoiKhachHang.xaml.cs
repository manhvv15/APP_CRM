...
{
    /// <summary>
    /// Interaction logic for frmThemMoiKhachHang.xaml
    /// </summary>
    public partial class frmThemMoiKhachHang : Window, INotifyPropertyChanged
    {
        ...
        private string TSD = "";
        private frmTrangChuSauDangNhapCongTy Main;

        public frmThemMoiKhachHang(frmTrangChuSauDangNhapCongTy main, string TieuDe, clsKhachHang.KhachHang khachhang, List<clsNhanVien.Item> listNV, List<clsTinhTrangKhachHang.Datum> listTT, List<clsNhomKHConMacDinh.Datum> listNhomKH, List<clsNhomKhachHangMacDinh.Datum> listNhomCha, DataLogin_Company data, List<clsNguonKhachHang.Datum> lstNguon, List<clsKhachHang.KhachHang> lstKH, string tongsodong, List<clsNganHang.Datum> listNH, List<clsDoanhThu.Datum> listDT, List<clsQuyMoNhanSu.Datum> listQMNS, List<clsXepHangKhachHang.Datum> listXHKH, List<clsLoaiHinhKhachHang.Datum> listLHKH, List<clsPhanLoaiKhachHang.Datum> listPLKH, List<clsLinhVucKH.Datum> listLVKH, List<clsNganhNgheKH.Datum> listNNKH, List<clsTinhThanh.Datum> listTTH,List<Class.clsPhongBanThuocCongTy.Item> lstPb)
        {
            InitializeComponent();
        ...
        {
            NhomKhachHang.frmNhomKhachHang frm = new NhomKhachHang.frmNhomKhachHang(dt, Main);
        ...
        private void bd_Luu_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            if (textTieuDe.Text == "Chỉnh sửa khách hàng")
            {
                using (RestClient restclient = new RestClient(new Uri("https://crm.timviec365.vn/ApiWinform/updateCustomer")))
                {
                    ...
                    Socket.UserDisconnect(dt.com_id);
                    frmCustomer frm = new frmCustomer(Main, dt, LstKhachHang, TSD, LstNguonKH, LstNhomKhachHangCha, LstNhomKhachHang, LstTinhTrang, ListNhanVien, ListPhanLoaiKH, ListLinhVuc, ListLoaiHinhKH, ListNganhNghe, ListTinhThanh, LstNganHang, LstDoanhThu, LstQuyMoNhanSu, LstXepHangKhachHang, LstPhongBan);
                    pnlHienThi.Children.Clear();
                    object Content = frm.Content;
                    frm.Content = null;
                    //frm.Close();
        ...
        {
            frmCustomer frm = new frmCustomer(dt, LstKhachHang, TSD, LstNguonKH, LstNhomKhachHangCha, LstNhomKhachHang, LstTinhTrang, ListNhanVien, ListPhanLoaiKH, ListLinhVuc, ListLoaiHinhKH, ListNganhNghe, ListTinhThanh, LstNganHang, LstDoanhThu, LstQuyMoNhanSu, LstXepHangKhachHang);
            pnlHienThi.Children.Clear();
            object Content = frm.Content;
            frm.Content = null;
            //frm.Close();
